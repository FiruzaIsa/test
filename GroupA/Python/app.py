...
def generate_report4(cve_infos):
    report =[]
    if cve_infos:
...
def index():
    if request.method == "POST":
        ...
        info =get_info_from_links(cve_id)
        ...
        if nvd_description:
            ...
            if info:
               report4=generate_report4(info)
            else:
               data_not_available = True
               report4="Not Found"
               
            return render_template("report.html", report=report,report2=report2,report3=report3,report4=report4,data_not_available=data_not_available)
